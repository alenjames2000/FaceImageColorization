...
class Net(nn.Module):
    def __init__(self):
        super().__init__()
        self.bm = nn.BatchNorm2d(3, track_running_stats=False)
...
for epoch in range(100):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        if(i == 63 and (epoch == 0 or epoch == 5 or epoch == 10 or epoch == 14 or epoch == 19)):
            cv2.imwrite(rf'../Predictions/ExampleColorizations/Epoch{epoch}TrainReal.jpg', cv2.cvtColor(torch.cat((torch.mul(inputs[i], 100), labels[i]), 0).permute(1,2,0).numpy(), cv2.COLOR_LAB2BGR)*255)
            cv2.imwrite(rf'../Predictions/ExampleColorizations/Epoch{epoch}TrainPred.jpg', cv2.cvtColor(torch.cat((torch.mul(inputs[i], 100), outputs[i]), 0).permute(1,2,0).detach().numpy(), cv2.COLOR_LAB2BGR)*255)
...
outputs = net(images)
for i in ([0,35,74]):
    cv2.imwrite(rf'../Predictions/ExampleColorizations/Test{i}Real.jpg', cv2.cvtColor(torch.cat((torch.mul(images[i], 100), labels[i]), 0).permute(1,2,0).numpy(), cv2.COLOR_LAB2BGR)*255)
    cv2.imwrite(rf'../Predictions/ExampleColorizations/Test{i}Pred.jpg', cv2.cvtColor(torch.cat((torch.mul(images[i], 100), outputs[i]), 0).permute(1,2,0).detach().numpy(), cv2.COLOR_LAB2BGR)*255)
